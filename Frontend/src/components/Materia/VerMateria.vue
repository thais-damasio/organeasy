            ...
            <div v-if="materia">
                <div class="uk-clearfix">
                    <div class="uk-float-left">
                        <h3 class="uk-card-title uk-margin-remove-bottom">{{materia.nome}}</h3>
                    </div>
                    <div class="uk-float-right">
                        <a @click.prevent.stop="apagarMateria()" class="i-remove" uk-icon="trash"></a>
                    </div>
                </div>
                <small v-if="materia.atualizado_em">Última atualização: {{materia.atualizado_em | formatDate}}</small>
            </div>
        </div>
        <!--  -->
        <div v-if="materia">
            <div class="uk-card-body">
                ...
                <ul v-if="materia.atividadesMateria" uk-accordion>
                    <li class="uk-open">
                        <a class="uk-accordion-title uk-button-accordion-li uk-background-muted uk-padding-small" href="#">
                            Atividades <small>({{materia.atividadesMateria.length}})</small>
                        </a>
                        <div class="uk-accordion-content">
                            <ul v-if="materia.atividadesMateria.length > 0" class="uk-list uk-list-striped">
                                <li v-for="(atv) in materia.atividadesMateria" :key="atv.id">
                                    ...
                                    <router-link :to="{ name: 'verAtividadeMateria', params: { id: atv.id}}">{{atv.atividade.titulo}}</router-link>
            ...
            <div class="uk-card-footer">
                <div class="uk-clearfix">
                    <div class="uk-float-left">
                        <router-link :to="{name: 'editarMateria', params: {id: materia.id}}" class="uk-button uk-button-primary">Editar</router-link>
                    </div>
                    <div class="uk-float-right">
        ...
        apagarMateria() {
            try {
                ...
                this.$swal.fire({
                    ...
                    text: "Todas as atividades associadas a essa matéria serão apagadas e você não será capaz de reverter essa ação!",
                    type: 'warning',
                    ...
                    if (result.value) {
                        ...
                        let response = await this.$http.delete(process.env.API_URL + 'materia/delete/' + this.id, {headers: {Authorization: token}});
